    ...
    resendMail: (req, res) => {
        User(req, (user, err) => {
            ...
            if (user.verification !== null) {
                if (user.ver_expire_date === null) {
                    VerificationController.sendMail(res, user)
                } else if (Date.now() >= user.ver_expire_date) {
                    VerificationController.sendMail(res, user)
                    // sendMail(user)
                } else {
                    response(res, req.body, {}, 403, "You can only resend mail every 30 minutes", []);
                };
            } else {
                response(res, req.body, {}, 403, "User already validated", [])
            }
        })
    },

    sendMail: (res, user) => {
        var minutes = 30 * 60000 // 30 minutes in ms
        user.ver_expire_date = Date.now() + minutes
        user.save();
        mailService.send(
            user.email,
        ...
        response(res, null, {}, 200, "Verification mail has been sent", false);
    }
}
