...
orm.connect(
  ...
  (err, db) => {
    ...
    const card = db.define("cards", {
      ...
      white: Boolean,
      picks: Number,
      cardpack_id: { type: "text", size: 255 }
    });
...
const getJson = callback => {
  var headers = {
    ...
    Host: "crhallberg.com",
    "Postman-Token":
      "8081718d-78cb-4fdd-929b-ba272120e593,786d1445-f21e-4167-baa7-e1df4182822f",
    accept:
    ...
    var bar = new progress(
      "adding cards [:bar] :rate/cards per second :percent :etas",
      {
        ...
        callback: () => {
          ...
          console.log(
            "COMPLETED: successfully seeded default cards and cardpacks!"
          );
          process.exit(0);
        }
      }
    ...
    models.card.create(
      ...
      (err, result) => {
        ...
        if (err) {
          ...
          addCard(models, cardpack, result);
        }
      }
    );
  }
...
const addCard = (models, cardpack, card) => {
  models.card.create(card, (err, res) => {
    if (err) {
      throw err;
    }
  });
